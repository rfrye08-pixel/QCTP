...
Local AI Mirror release authority is specifically held until a model/runtime is provisioned locally, a representative grounded Mirror workload is benchmarked on Ryan's actual PX13, and the complete private-HTTPS queue/reconnect/result/review/deletion workflow is accepted on the physical target iPhone. Mock-provider and desktop-browser results prove contracts only; they do not satisfy those hardware holds.
